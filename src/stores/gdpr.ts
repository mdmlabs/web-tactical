...
export const useGdprStore = defineStore("gdpr", () => {
  ...
  const inspectedEvent = ref<GDPREvent | null>(null);
  const showDetailPanel = ref(false);
  const surroundingEvents = ref<GDPREvent[]>([]);
  ...
  const groupedArticleCounts = computed(() => {
    ...
    for (const rc of articleCounts.value) {
      const parentId = rc.article.split(".")[0];
      map.set(parentId, (map.get(parentId) ?? 0) + rc.doc_count);
    }
    return [...map.entries()]
      .sort((a, b) => b[1] - a[1])
      .map(([article, doc_count]) => ({ article, doc_count }));
  });

  const filteredArticleCounts = computed(() => {
    const q = controlsSearch.value.trim().toLowerCase();
    if (!q) return articleCounts.value;
    return articleCounts.value.filter((r) =>
  ...
  function buildEventsMusts(): Record<string, unknown>[] {
    ...
    if (selectedArticle.value) {
      must.push({ match: { "rule.gdpr": selectedArticle.value } });
    }
  ...
  async function fetchAgentsList() {
    ...
    try {
      ...
      console.error("[GDPR] Failed to fetch agents list:", e);
    } finally {
      agentsListLoading.value = false;
    }
  }

  function setSelectedAgent(agent: GDPRSelectedAgent | null) {
  ...
  async function fetchDashboard() {
    ...
    try {
      ...
      const aggs: Record<string, any> = {
        top_articles: {
          terms: { field: "rule.gdpr", size: agentId ? 5 : 10 },
        },
        trend: {
          date_histogram: {
            field: "@timestamp",
      ...
      const topArticles: GDPRArticleCount[] = (
        respAggs?.top_articles?.buckets ?? []
      ).map((b: { key: string; doc_count: number }) => ({
        article: b.key,
        doc_count: b.doc_count,
      }));

      const trend: GDPRTrendBucket[] = (
        ...
        (b: {
          ...
          doc_count: number;
          by_article: { buckets: { key: string; doc_count: number }[] };
        }) => ({
          key: b.key,
          key_as_string: b.key_as_string,
          doc_count: b.doc_count,
          by_article: b.by_article,
        }),
      );

      const result: GDPRDashboardAggs = {
        topArticles,
        topAgents: [],
        trend,
      ...
      console.error("[GDPR] Dashboard fetch error:", e);
      Notify.create({
        type: "negative",
        message: "Failed to load GDPR dashboard data",
        timeout: 3000,
      });
    } finally {
  ...
  // === Controls: fetch all article counts ===
  async function fetchArticleCounts() {
    ...
    try {
      ...
      const body: OpenSearchQueryBody = {
        ...
        aggs: {
          all_articles: {
            terms: { field: "rule.gdpr", size: 200 },
          },
        },
      };
      ...
      indexerAvailable.value = false;
      console.error("[GDPR] Controls fetch error:", e);
      Notify.create({
        type: "negative",
        message: "Failed to load GDPR articles",
        timeout: 3000,
      });
    } finally {
  ...
  async function fetchEvents() {
    ...
    try {
      ...
      const resp = await wazuhIndexerApi.search<GDPREvent>(
        "wazuh-alerts-*",
        body,
      );
      ...
      indexerAvailable.value = false;
      console.error("[GDPR] Events fetch error:", e);
      Notify.create({
        type: "negative",
        message: "Failed to load GDPR events",
        timeout: 3000,
      });
    } finally {
  ...
  async function fetchEventsHistogram() {
    ...
    try {
      ...
      console.error("[GDPR] Histogram fetch error:", e);
    } finally {
  ...
  async function fetchDetailEvents() {
    ...
    try {
      ...
      console.error("[GDPR] Detail events fetch error:", e);
    } finally {
      detailEventsLoading.value = false;
    }
  }

  // === Article detail: SCA checks ===
  async function fetchDetailSCA() {
    ...
    try {
        ...
        agents.map(async (agent) => {
          ...
          for (const policy of policies) {
            ...
            for (const check of checksResp.data.affected_items) {
              if (check.compliance) {
                const hasGdprMatch = check.compliance.some(
                  (c) =>
                    c.key === "gdpr_IV" &&
                    c.value === selectedArticle.value,
                );
                if (hasGdprMatch) {
      ...
      console.error("[GDPR] SCA checks fetch error:", e);
    } finally {
  ...
  // === Document inspection ===
  function inspectDocument(event: GDPREvent) {
  ...
  async function fetchSurroundingEvents() {
    ...
    try {
      ...
      const [beforeResp, afterResp] = await Promise.all([
        wazuhIndexerApi.search<GDPREvent>("wazuh-alerts-*", {
          query: {
            bool: {
              must: [
                ...must,
      ...
      console.error("[GDPR] Surrounding events fetch error:", e);
    } finally {
  ...
  // === Filter helpers ===
  function filterToClause(f: GDPRFilter): Record<string, unknown> | null {
  ...
  function addFilter(filter: Omit<GDPRFilter, "id">) {
  ...
  return {
    ...
    // Getters
    dateRangeQuery,
    groupedArticleCounts,
    filteredArticleCounts,
    hasAgent,
