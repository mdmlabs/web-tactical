...
export const useTscStore = defineStore("tsc", () => {
  ...
  const groupedCriteriaCounts = computed(() => {
    ...
    for (const rc of criteriaCounts.value) {
      // Group by prefix: CC6.1 -> CC6, A1.2 -> A1, PI1.3 -> PI1
      const match = rc.criteria.match(/^([A-Z]+\d+)/);
      const groupId = match ? match[1] : rc.criteria;
      map.set(groupId, (map.get(groupId) ?? 0) + rc.doc_count);
    }
    return [...map.entries()]
      .sort((a, b) => b[1] - a[1])
      .map(([criteria, doc_count]) => ({ criteria, doc_count }));
  });

  const filteredCriteriaCounts = computed(() => {
    const q = controlsSearch.value.trim().toLowerCase();
    if (!q) return criteriaCounts.value;
    return criteriaCounts.value.filter((r) =>
  ...
  function buildEventsMusts(): Record<string, unknown>[] {
    ...
    if (selectedCriteria.value) {
      must.push({ match: { "rule.tsc": selectedCriteria.value } });
    }
  ...
  async function fetchAgentsList() {
    ...
    try {
      ...
      console.error("[TSC] Failed to fetch agents list:", e);
    } finally {
      agentsListLoading.value = false;
    }
  }

  function setSelectedAgent(agent: TSCSelectedAgent | null) {
  ...
  function selectCriteria(criteriaId: string | null) {
    selectedCriteria.value = criteriaId;
    detailEvents.value = [];
  ...
  async function fetchDashboard() {
    ...
    try {
      ...
      const aggs: Record<string, any> = {
        top_criteria: {
          terms: { field: "rule.tsc", size: agentId ? 5 : 10 },
        },
        trend: {
          date_histogram: {
            field: "@timestamp",
      ...
      const topCriteria: TSCCriteriaCount[] = (
        respAggs?.top_criteria?.buckets ?? []
      ).map((b: { key: string; doc_count: number }) => ({
        criteria: b.key,
        doc_count: b.doc_count,
      }));

      const trend: TSCTrendBucket[] = (
        ...
        (b: {
          ...
          doc_count: number;
          by_criteria: { buckets: { key: string; doc_count: number }[] };
        }) => ({
          key: b.key,
          key_as_string: b.key_as_string,
          doc_count: b.doc_count,
          by_criteria: b.by_criteria,
        }),
      );

      const result: TSCDashboardAggs = {
        topCriteria,
        topAgents: [],
        trend,
      ...
      console.error("[TSC] Dashboard fetch error:", e);
      Notify.create({
        type: "negative",
        message: "Failed to load TSC dashboard data",
        timeout: 3000,
      });
    } finally {
  ...
  // === Controls: fetch all criteria counts ===
  async function fetchCriteriaCounts() {
    ...
    try {
      ...
      criteriaCounts.value = (
        aggs?.all_criteria?.buckets ?? []
      ).map((b: { key: string; doc_count: number }) => ({
        criteria: b.key,
        doc_count: b.doc_count,
      }));
      ...
      indexerAvailable.value = false;
      console.error("[TSC] Controls fetch error:", e);
      Notify.create({
        type: "negative",
        message: "Failed to load TSC criteria",
        timeout: 3000,
      });
    } finally {
  ...
  async function fetchEvents() {
    ...
    try {
      ...
      const resp = await wazuhIndexerApi.search<TSCEvent>(
        "wazuh-alerts-*",
        body,
      );
      ...
      indexerAvailable.value = false;
      console.error("[TSC] Events fetch error:", e);
      Notify.create({
        type: "negative",
        message: "Failed to load TSC events",
        timeout: 3000,
      });
    } finally {
  ...
  async function fetchDetailEvents() {
    ...
    try {
      ...
      console.error("[TSC] Detail events fetch error:", e);
    } finally {
      detailEventsLoading.value = false;
    }
  }

  // === Criteria detail: SCA checks ===
  async function fetchDetailSCA() {
    ...
    try {
        ...
        agents.map(async (agent) => {
          ...
          for (const policy of policies) {
            ...
            for (const check of checksResp.data.affected_items) {
              if (check.compliance) {
                const hasTscMatch = check.compliance.some(
                  (c) =>
                    c.key === "tsc" &&
                    c.value === selectedCriteria.value,
                );
                if (hasTscMatch) {
      ...
      console.error("[TSC] SCA checks fetch error:", e);
    } finally {
  ...
  // === Document inspection ===
  function inspectDocument(event: TSCEvent) {
  ...
  async function fetchSurroundingEvents() {
    ...
    try {
      ...
      const [beforeResp, afterResp] = await Promise.all([
        wazuhIndexerApi.search<TSCEvent>("wazuh-alerts-*", {
          query: {
            bool: {
              must: [
                ...must,
      ...
      console.error("[TSC] Surrounding events fetch error:", e);
    } finally {
  ...
  function addFilter(filter: Omit<TSCFilter, "id">) {
    ...
    return JSON.stringify(
      {
        ...
        criteriaCounts: criteriaCounts.value,
      },
      null,
      2,
    );
  ...
  return {
    ...
    // Getters
    dateRangeQuery,
    groupedCriteriaCounts,
    filteredCriteriaCounts,
    hasAgent,
