...
export const useNist80053Store = defineStore("nist80053", () => {
  ...
  const groupedControlCounts = computed(() => {
    ...
    for (const rc of controlCounts.value) {
      ...
      const familyId =
        dashIdx > 0 ? rc.control.substring(0, dashIdx) : rc.control;
    ...
    try {
      ...
      const aggs: Record<string, any> = {
        ...
        trend: {
          ...
          aggs: { by_control: { terms: { field: "rule.nist_800_53", size: 5 } } },
        },
      };
      ...
      if (!agentId) {
        ...
        aggs.top_rule_groups = { terms: { field: "rule.groups", size: 5 } };
        aggs.top_rules = {
          terms: { field: "rule.id", size: 5 },
          aggs: {
            rule_desc: { terms: { field: "rule.description.keyword", size: 1 } },
            rule_desc_fallback: { terms: { field: "rule.description", size: 1 } },
          },
        };
        aggs.rule_level_dist = { terms: { field: "rule.level", size: 20 } };
      }
      ...
      if (!agentId) {
        result.topAgents = (respAggs?.top_agents?.buckets ?? []).map(
          (b: { key: string; doc_count: number; agent_id: { buckets: { key: string }[] } }) => ({
            agent_name: b.key,
        ...
        result.topRuleGroups = (respAggs?.top_rule_groups?.buckets ?? []).map(
          (b: { key: string; doc_count: number }) => ({ group: b.key, doc_count: b.doc_count }),
        ) as NIST80053RuleGroupCount[];

        result.topRules = (respAggs?.top_rules?.buckets ?? []).map(
          (b: { key: string; doc_count: number; rule_desc: { buckets: { key: string }[] }; rule_desc_fallback: { buckets: { key: string }[] } }) => ({
            rule_id: String(b.key),
            description: b.rule_desc?.buckets?.[0]?.key ?? b.rule_desc_fallback?.buckets?.[0]?.key ?? `Rule ${b.key}`,
            doc_count: b.doc_count,
          }),
        ...
        const totalLevelCount = levelBuckets.reduce(
          (sum: number, b: { doc_count: number }) => sum + b.doc_count, 0,
        );
        result.ruleLevelDistribution = levelBuckets.map(
          (b: { key: number; doc_count: number }) => ({
            level: b.key,
            doc_count: b.doc_count,
            percentage: totalLevelCount > 0 ? Math.round((b.doc_count / totalLevelCount) * 10000) / 100 : 0,
          }),
      ...
      Notify.create({ type: "negative", message: "Failed to load NIST 800-53 dashboard data", timeout: 3000 });
    } finally {
  ...
  async function fetchControlCounts() {
    ...
    try {
      ...
      const body: OpenSearchQueryBody = {
        query: { bool: { must } },
        size: 0,
        aggs: { all_controls: { terms: { field: "rule.nist_800_53", size: 500 } } },
      };
      const resp = await wazuhIndexerApi.search("wazuh-alerts-*", body);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const aggs = resp.aggregations as any;
      controlCounts.value = (aggs?.all_controls?.buckets ?? []).map(
        (b: { key: string; doc_count: number }) => ({ control: b.key, doc_count: b.doc_count }),
      );
      indexerAvailable.value = true;
    } catch (e) {
      indexerAvailable.value = false;
      console.error("[NIST 800-53] Controls fetch error:", e);
      Notify.create({ type: "negative", message: "Failed to load NIST 800-53 controls", timeout: 3000 });
    } finally {
  ...
  async function fetchEvents() {
    ...
    try {
      ...
      const from = (eventsPagination.value.page - 1) * eventsPagination.value.rowsPerPage;
      const body: OpenSearchQueryBody = {
        query: { bool: { must } },
      ...
      const resp = await wazuhIndexerApi.search<NIST80053Event>("wazuh-alerts-*", body);
      events.value = resp.hits.hits.map((h) => ({ _id: h._id, _index: h._index, _source: h._source }));
      eventsTotal.value = resp.hits.total.value;
      indexerAvailable.value = true;
    } catch (e) {
      indexerAvailable.value = false;
      console.error("[NIST 800-53] Events fetch error:", e);
      Notify.create({ type: "negative", message: "Failed to load NIST 800-53 events", timeout: 3000 });
    } finally {
  ...
  async function fetchDetailEvents() {
    ...
    try {
      ...
      const from = (detailEventsPagination.value.page - 1) * detailEventsPagination.value.rowsPerPage;
      const body: OpenSearchQueryBody = {
        query: { bool: { must } },
      ...
      const resp = await wazuhIndexerApi.search<NIST80053Event>("wazuh-alerts-*", body);
      detailEvents.value = resp.hits.hits.map((h) => ({ _id: h._id, _index: h._index, _source: h._source }));
  ...
  async function fetchDetailSCA() {
    ...
    try {
      ...
      if (selectedAgent.value) {
        agents = agentsList.value.filter((a) => a.id === selectedAgent.value!.id && a.status === "active");
      } else {
        agents = wazuhStore.wazuhAgents.filter((a) => a.id !== "000" && a.status === "active");
      }
      ...
      const results = await Promise.allSettled(
        agents.map(async (agent) => {
          ...
          for (const policy of policies) {
            const checksResp = await wazuhApi.getSCAChecks(agent.id, policy.policy_id);
            for (const check of checksResp.data.affected_items) {
              if (check.compliance) {
                const hasNistMatch = check.compliance.some(
                  (c) => c.key === "nist_800_53" && c.value === selectedControl.value,
                );
                if (hasNistMatch) agentChecks.push(check);
              }
            }
          }
          return agentChecks;
  ...
  async function fetchSurroundingEvents() {
    ...
    try {
      ...
      if (!ts) return;
      const must: Record<string, unknown>[] = [{ exists: { field: "rule.nist_800_53" } }];
      ...
      const [beforeResp, afterResp] = await Promise.all([
        wazuhIndexerApi.search<NIST80053Event>("wazuh-alerts-*", {
          query: { bool: { must: [...must, { range: { "@timestamp": { lt: ts } } }] } },
          size: 5,
          sort: [{ "@timestamp": { order: "desc" } }],
        }),
        wazuhIndexerApi.search<NIST80053Event>("wazuh-alerts-*", {
          query: { bool: { must: [...must, { range: { "@timestamp": { gt: ts } } }] } },
          size: 5,
      ...
      const before = (beforeResp.hits.hits ?? [])
        ...
        .reverse();
      const after = (afterResp.hits.hits ?? []).map((h) => ({ _id: h._id, _index: h._index, _source: h._source }));
  ...
  function filterToClause(f: NIST80053Filter): Record<string, unknown> | null {
    ...
    switch (f.operator) {
      case "is": clause = { match: { [f.field]: f.value } }; break;
      case "is_not": clause = { bool: { must_not: [{ match: { [f.field]: f.value } }] } }; break;
      case "exists": clause = { exists: { field: f.field } }; break;
      case "does_not_exist": clause = { bool: { must_not: [{ exists: { field: f.field } }] } }; break;
    }
  ...
  function addFilter(filter: Omit<NIST80053Filter, "id">) { eventsFilters.value.push({ ...filter, id: generateId() }); fetchEvents(); }
  ...
  function exportDashboardJSON(): string {
    return JSON.stringify({ timestamp: new Date().toISOString(), dateRange: dateRange.value, selectedAgent: selectedAgent.value, dashboard: dashboardAggs.value, controlCounts: controlCounts.value }, null, 2);
  }

  return {
    activeTab, selectedControl, dateRange, indexerAvailable, selectedAgent, agentsList, agentsListLoading,
    dashboardAggs, dashboardLoading, controlCounts, controlsLoading, controlsSearch, controlsHideNoAlerts,
    detailEvents, detailEventsTotal, detailEventsLoading, detailEventsPagination, detailSCAChecks, detailSCALoading, detailSubTab,
    events, eventsTotal, eventsLoading, eventsPagination, eventsSearch, eventsFilters, eventsHistogramData, eventsHistogramLoading,
    inspectedEvent, showDetailPanel, surroundingEvents, surroundingEventsLoading, showRawJson,
    dateRangeQuery, groupedControlCounts, filteredControlCounts, hasAgent,
    setActiveTab, setDateRange, refreshActiveTab, fetchAgentsList, setSelectedAgent, selectControl, clearControlSelection,
    fetchDashboard, fetchControlCounts, fetchEvents, fetchEventsHistogram, fetchDetailEvents, fetchDetailSCA,
    addFilter, removeFilter, clearFilters, setEventsPage, setEventsRowsPerPage, setEventsSearch, setDetailEventsPage,
    inspectDocument, closeInspection, fetchSurroundingEvents, exportDashboardJSON,
  };
});
