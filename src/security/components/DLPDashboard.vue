<template>
  <div class="dlp-dashboard">
    ...
    <div class="row q-gutter-md q-mb-lg">
      ...
      <q-card class="col-12 col-sm-5 col-md-2" flat bordered>
        <q-card-section class="text-center q-pa-md">
          <q-icon name="task_alt" size="2rem" color="primary" />
          <div class="text-h4 text-weight-bold text-primary q-mt-xs">{{ coveredCount }}/10</div>
          <div class="text-caption text-grey-7">{{ $t('security.components.DLPDashboard.cf7a4b') }}</div>
        </q-card-section>
      </q-card>
    </div>
    ...
    <div class="row q-gutter-md q-mb-lg">
      ...
      <!-- Compliance coverage STT 842-851 -->
      <q-card class="col-12 col-md-5" flat bordered>
        <q-card-section>
          <div class="text-subtitle2 q-mb-sm">{{ $t('security.components.DLPDashboard.853804') }}</div>
          <div v-for="(status, stt) in report.compliance_coverage" :key="stt" class="row items-center q-mb-xs">
            <div class="col-3 text-caption text-weight-medium">#{{ stt }}</div>
            <div class="col-7 text-caption text-grey-8">{{ sttLabels[stt] || '' }}</div>
            <q-chip
              :color="coverageColor(status)"
              text-color="white"
              dense size="sm"
              :label="status === 'covered' ? 'Covered' : status === 'partial' ? 'Partial' : 'Missing'"
            />
          </div>
        </q-card-section>
      </q-card>
...
const coveredCount = computed(() => {
  const cov = report.value.compliance_coverage || {}
  return Object.values(cov).filter(v => v === 'covered').length
})
...
async function loadReport() {
  loading.value = true
  try {
    const resp = await axios.get('/security/dlp/report/', { params: { period: period.value } })
