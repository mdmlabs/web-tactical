  ...
  <q-card style="width: 100%; max-width: 1200px;">
      ...
      <div class="row items-center justify-between">
        <div>
          ...
          <div class="text-caption">
            Запущено: {{ jobInfo.started }} назад
          </div>
        </div>
    ...
    <q-card-section>
      <div class="text-subtitle1 q-mb-sm">
        Целевая группа: <strong>{{ jobInfo.target }}</strong> ({{ results.length }} устройств)
      </div>
          ...
          <q-td :props="props">
            <div class="row items-center">
              ...
              <div>
                ...
                <div class="text-caption text-grey">{{ props.row.hostname }}</div>
              </div>
            </div>
          </q-td>
        </template>
        ...
        <q-card-section>
          ...
          <q-list dense>
            <q-item v-for="(error, index) in errors" :key="index">
              ...
              <q-item-section>
                <q-item-label>{{ error.device }}: {{ error.message }}</q-item-label>
      ...
      <q-card>
        <q-card-section class="bg-dark text-white">
          <div class="row items-center justify-between">
            ...
            <q-btn flat round dense icon="close" @click="showLogsDialog = false" />
          </div>
        </q-card-section>

        <q-card-section class="bg-grey-10 text-white" style="font-family: monospace; height: 70vh; overflow-y: auto;">
          <div v-for="(log, index) in selectedAgent?.logs" :key="index" class="q-mb-sm">
...
export default {
  ...
  setup() {
    ...
    const results = ref([
      {
        ...
        message: 'Установлено',
        logs: [
          { timestamp: '10:15:23', level: 'info', message: 'Начало загрузки файла...' },
          { timestamp: '10:15:45', level: 'info', message: 'Загружено 2.3 GB' },
          { timestamp: '10:15:46', level: 'info', message: 'Проверка хэша: OK' },
          { timestamp: '10:15:47', level: 'info', message: 'Запуск установки...' },
          { timestamp: '10:18:12', level: 'success', message: 'Установка завершена успешно' },
          { timestamp: '10:18:13', level: 'success', message: 'Проверка: найден в реестре' },
        ],
      },
      {
        id: 2,
      ...
      },
      {
        ...
        message: 'Установка...',
        logs: [
          { timestamp: '10:16:01', level: 'info', message: 'Начало загрузки файла...' },
          { timestamp: '10:16:23', level: 'info', message: 'Загружено 2.3 GB' },
          { timestamp: '10:16:24', level: 'info', message: 'Запуск установки...' },
        ],
      },
      {
        id: 4,
        ...
        logs: [
          { timestamp: '10:17:45', level: 'info', message: 'Начало загрузки файла...' },
        ],
      },
      {
        id: 5,
      ...
      },
      {
        ...
        logs: [
          { timestamp: '10:15:10', level: 'error', message: 'Не удалось подключиться к агенту' },
          { timestamp: '10:15:10', level: 'error', message: 'Агент не в сети (последний раз: 2 часа назад)' },
        ],
      },
      {
        id: 7,
        ...
        logs: [
          { timestamp: '10:15:12', level: 'info', message: 'Проверка требований...' },
          { timestamp: '10:15:13', level: 'error', message: 'Недостаточно места на диске' },
          { timestamp: '10:15:13', level: 'error', message: 'Требуется: 5 GB, доступно: 45 MB' },
        ],
      },
    ]);
    ...
    const inProgressCount = computed(() => {
      return results.value.filter(r => ['downloading', 'installing', 'verifying'].includes(r.status)).length;
    });
