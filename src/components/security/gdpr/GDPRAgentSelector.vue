...
const selectedAgent = ref<string | null>(gdprStore.selectedAgent?.id ?? null);
...
function onAgentChange(agentId: string | null) {
  if (agentId === null) {
    gdprStore.setSelectedAgent(null);
  } else {
    const agent = wazuhStore.wazuhAgents.find((a) => a.id === agentId);
    if (agent) {
      gdprStore.setSelectedAgent({ id: agent.id, name: agent.name });
    }
  }
}
