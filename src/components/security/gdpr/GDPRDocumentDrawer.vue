<template>
  <q-drawer
    ...
    bordered
    overlay
    class="gdpr-doc-drawer"
    @update:model-value="onDrawerToggle"
  >
    <div v-if="store.inspectedEvent" class="gdpr-doc-content">
      <!-- Header -->
      <div class="gdpr-doc-header">
        <span class="gdpr-doc-title">Document Details</span>
        <div class="gdpr-doc-actions">
          <q-btn
            flat
            dense
            no-caps
      ...
      <!-- Raw JSON view -->
      <div v-if="viewTab === 'json'" class="gdpr-doc-json">
        <pre class="gdpr-json-pre">{{ formattedJson }}</pre>
      </div>

      <!-- Table view -->
      <div v-else class="gdpr-doc-table-wrap">
        <q-list dense separator class="gdpr-doc-field-list">
          <q-item
            v-for="([key, value], idx) in flatEntries"
            :key="idx"
            class="gdpr-doc-field-item"
          >
            <q-item-section side class="gdpr-doc-field-num">
              {{ idx + 1 }}
            </q-item-section>
            <q-item-section class="gdpr-doc-field-key">
              {{ key }}
            </q-item-section>
            <q-item-section class="gdpr-doc-field-value">
              {{ formatValue(value) }}
              <q-btn
      ...
      <template v-if="store.surroundingEvents.length > 0">
        <q-separator />
        <div class="gdpr-doc-surrounding">
          <div class="gdpr-doc-surrounding-title">Surrounding Documents</div>
          <q-list dense separator>
            <q-item
              v-for="(evt, idx) in store.surroundingEvents"
              :key="idx"
              clickable
              :class="{ 'gdpr-surrounding-current': evt._id === store.inspectedEvent?._id }"
              @click="store.inspectDocument(evt)"
            >
              <q-item-section>
                <q-item-label class="gdpr-surrounding-ts">
...
.gdpr-doc-field-item:hover .gdpr-doc-copy-btn {
  opacity: 0.7;
}
...
.gdpr-doc-surrounding {
  max-height: 300px;
  overflow-y: auto;
  padding: 8px 0;
}

.gdpr-doc-surrounding-title {
  font-size: 13px;
  font-weight: 600;
  color: var(--mdm-text-primary, #1a1a1a);
...
.gdpr-surrounding-current {
  background: rgba(37, 99, 235, 0.08);
}

/* Dark mode */
.body--dark .gdpr-doc-title,
.body--dark .gdpr-doc-field-key,
.body--dark .gdpr-doc-surrounding-title,
.body--dark .gdpr-json-pre {
