...
const selectedAgent = ref<string | null>(tscStore.selectedAgent?.id ?? null);
...
function onAgentChange(agentId: string | null) {
  if (agentId === null) {
    tscStore.setSelectedAgent(null);
  } else {
    const agent = wazuhStore.wazuhAgents.find((a) => a.id === agentId);
    if (agent) {
      tscStore.setSelectedAgent({ id: agent.id, name: agent.name });
    }
  }
}
