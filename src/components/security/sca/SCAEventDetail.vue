      ...
      <div class="detail-header">
        ...
        <div class="detail-links">
          <q-btn
            ...
            class="text-primary detail-link-btn"
        ...
        >
          <template #body-cell-type="props">
            <q-td :props="props">
              <q-badge
                ...
                :color="props.row.type === 't' ? 'blue-grey' : 'teal'"
                outline
                dense
              />
            </q-td>
          ...
          <template #body-cell-value="props">
            <q-td :props="props" class="field-value">
              <span
                v-if="isFilterable(props.row.field)"
                class="text-primary cursor-pointer"
                @click="$emit('add-filter', props.row.field, String(props.row.value))"
              >{{ props.row.value }}</span>
              <span v-else>{{ props.row.value }}</span>
              <q-btn
                v-if="props.row.field === 'data.sca.scan_id'"
                flat round dense size="xs" icon="search"
                class="q-ml-xs"
                @click="$emit('view-surrounding', event)"
              />
              <q-btn
                v-if="props.row.field === 'data.sca.scan_id'"
                flat round dense size="xs" icon="description"
                class="q-ml-xs"
                @click="$emit('view-single', event)"
              />
            </q-td>
...
const props = defineProps<{ event: SCAEvent }>();
defineEmits<{
  close: [];
...
const flattenedFields = computed<FlatField[]>(() => {
  ...
  flatten(source as Record<string, unknown>, "");
  return result;
});

const formattedJson = computed(() => {
  return JSON.stringify(
    { _index: props.event._index, _id: props.event._id, _source: props.event._source },
    null,
    2,
  );
});
...
const filterableFields = new Set([
  "agent.id", "agent.name", "data.sca.check.result", "rule.level",
]);

function isFilterable(field: string): boolean {
  return filterableFields.has(field);
}
