<template>
  <div class="pcidss-agent-selector">
    <q-select
...
const pciDssStore = usePciDssStore();

const selectedAgent = ref<string | null>(pciDssStore.selectedAgent?.id ?? null);
...
function onAgentChange(agentId: string | null) {
  if (agentId === null) {
    pciDssStore.setSelectedAgent(null);
  } else {
    const agent = wazuhStore.wazuhAgents.find((a) => a.id === agentId);
    if (agent) {
      pciDssStore.setSelectedAgent({ id: agent.id, name: agent.name });
    }
  }
}
