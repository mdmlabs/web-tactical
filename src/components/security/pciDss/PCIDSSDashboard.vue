<template>
  <div class="pcidss-dashboard">
    <!-- Loading -->
    <div v-if="store.dashboardLoading" class="pcidss-center">
      <q-spinner-dots color="primary" size="48px" />
      <p class="q-mt-md text-grey-7">Loading PCI DSS dashboard...</p>
    </div>
    ...
    <template v-else-if="store.dashboardAggs">
      ...
      <template v-if="!store.hasAgent">
        <div class="pcidss-grid-2">
          <!-- Top 10 Requirements: Bubble chart -->
          <q-card flat bordered class="pcidss-card">
            <q-card-section class="pcidss-card-header">
              <div class="pcidss-card__title">Top 10 PCI DSS requirements</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
              <div v-if="!store.dashboardAggs.topRequirements.length" class="text-grey text-center q-pa-md">
                No data available
              </div>
              <apexchart
                v-else
                type="bar"
                height="320"
                :options="topReqChartOptions"
                :series="topReqChartSeries"
              />
            </q-card-section>
          </q-card>
          ...
          <q-card flat bordered class="pcidss-card">
            <q-card-section class="pcidss-card-header">
              <div class="pcidss-card__title">Top 10 agents by alerts count</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
      ...
      <!-- ======== AGENT-SPECIFIC VIEW ======== -->
      <template v-else>
        <div class="pcidss-grid-3">
          <!-- Top 5 Rule Groups: Donut -->
          <q-card flat bordered class="pcidss-card">
            <q-card-section class="pcidss-card-header">
              <div class="pcidss-card__title">Top 5 rule groups</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
          ...
          <q-card flat bordered class="pcidss-card">
            <q-card-section class="pcidss-card-header">
              <div class="pcidss-card__title">Top 5 rules</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
          ...
          <!-- Top 5 PCI DSS Requirements: Donut -->
          <q-card flat bordered class="pcidss-card">
            <q-card-section class="pcidss-card-header">
              <div class="pcidss-card__title">Top 5 PCI DSS requirements</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
              <div v-if="!store.dashboardAggs.topRequirements.length" class="text-grey text-center q-pa-md">
                No data available
              ...
              <apexchart
                ...
                type="donut"
                height="260"
                :options="topReqDonutOptions"
                :series="topReqDonutSeries"
              />
            </q-card-section>
          </q-card>
        </div>
        ...
        <div class="pcidss-grid-2">
          <!-- PCI DSS Requirements: Bar chart -->
          <q-card flat bordered class="pcidss-card">
            <q-card-section class="pcidss-card-header">
              <div class="pcidss-card__title">PCI DSS requirements</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
              <apexchart
                v-if="store.dashboardAggs.topRequirements.length"
                type="bar"
                height="260"
                :options="pciReqBarOptions"
                :series="pciReqBarSeries"
              />
          ...
          <q-card flat bordered class="pcidss-card">
            <q-card-section class="pcidss-card-header">
              <div class="pcidss-card__title">Rule level distribution</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
      ...
      <q-card flat bordered class="pcidss-card pcidss-card--full">
        <q-card-section class="pcidss-card-header">
          <div class="pcidss-card__title">Alerts trend over time</div>
        </q-card-section>
        <q-card-section class="q-pt-none">
...
// Top 10 Requirements - Bar chart
const topReqChartOptions = computed(() => ({
  ...baseChartOptions,
  ...
  xaxis: {
    categories: store.dashboardAggs?.topRequirements.map((r) => r.requirement) ?? [],
...
const topReqChartSeries = computed(() => [
  {
    name: "Alerts",
    data: store.dashboardAggs?.topRequirements.map((r) => r.doc_count) ?? [],
  },
]);
...
// Top 5 PCI DSS Requirements (agent view) - Donut
const topReqDonutOptions = computed(() => ({
  ...baseChartOptions,
  chart: { ...baseChartOptions.chart, type: "donut" as const },
  labels: store.dashboardAggs?.topRequirements.map((r) => r.requirement) ?? [],
  legend: { position: "right" as const, fontSize: "11px" },
  dataLabels: { enabled: false },
  plotOptions: { pie: { donut: { size: "50%" } } },
}));

const topReqDonutSeries = computed(
  () => store.dashboardAggs?.topRequirements.map((r) => r.doc_count) ?? [],
);

// PCI DSS Requirements bar chart (agent view, bottom left)
const pciReqBarOptions = computed(() => ({
  ...baseChartOptions,
  chart: { ...baseChartOptions.chart, type: "bar" as const },
  plotOptions: {
  ...
  xaxis: {
    categories: store.dashboardAggs?.topRequirements.map((r) => r.requirement) ?? [],
    title: { text: "PCI DSS Requirements" },
    labels: { style: { fontSize: "11px" } },
  },
  yaxis: { title: { text: "Count" }, labels: { style: { fontSize: "11px" } } },
...
const pciReqBarSeries = computed(() => [
  {
    name: "Alerts",
    data: store.dashboardAggs?.topRequirements.map((r) => r.doc_count) ?? [],
  },
