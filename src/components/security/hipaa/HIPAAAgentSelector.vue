<template>
  <div class="hipaa-agent-selector">
    <q-select
...
const hipaaStore = useHipaaStore();

const selectedAgent = ref<string | null>(hipaaStore.selectedAgent?.id ?? null);
...
function onAgentChange(agentId: string | null) {
  if (agentId === null) {
    hipaaStore.setSelectedAgent(null);
  } else {
    const agent = wazuhStore.wazuhAgents.find((a) => a.id === agentId);
    if (agent) {
      hipaaStore.setSelectedAgent({ id: agent.id, name: agent.name });
    }
  }
}
