<template>
  <div class="hipaa-events">
    <!-- Top bar with filters -->
    <div class="hipaa-events-toolbar">
      <div class="hipaa-events-filters">
        <!-- Default filter chips -->
        <q-chip
          size="sm"
          color="grey-3"
          text-color="grey-8"
          class="hipaa-filter-chip"
        >
        ...
        <q-btn flat dense no-caps size="sm" icon="add" label="Add filter" class="q-ml-xs">
          <q-popup-proxy>
            <q-card class="hipaa-add-filter-card">
    ...
    <!-- Histogram -->
    <div class="hipaa-events-histogram">
      <div v-if="store.eventsHistogramLoading" class="hipaa-histogram-loading">
      ...
      />
      <div class="hipaa-histogram-label">
        timestamp per 30 minutes
      </div>
    </div>

    <!-- Results info -->
    <div class="hipaa-events-info">
      <span class="hipaa-events-count">
        {{ store.eventsTotal.toLocaleString() }} hits
      </span>
      <span v-if="timeRange" class="hipaa-events-time-range">
        {{ timeRange }}
      </span>
    </div>

    <!-- Loading -->
    <div v-if="store.eventsLoading" class="hipaa-center">
      <q-spinner-dots color="primary" size="36px" />
    </div>

    <!-- Events table -->
    <div v-else class="hipaa-events-table-wrap">
      <q-table
        :rows="tableRows"
        :columns="columns"
        row-key="_id"
      ...
      >
        ...
        <template #body-cell-hipaa="props">
          <q-td :props="props">
            <q-badge
              v-for="standard in normalizeArray(props.row.hipaa)"
              :key="standard"
              color="blue-2"
              text-color="blue-9"
              :label="standard"
      ...
      <!-- Pagination -->
      <div class="hipaa-events-pagination">
        <q-select
...
import { HIPAA_KNOWN_FIELDS } from "@/types/hipaa";
...
const filterFieldOptions = HIPAA_KNOWN_FIELDS;
...
const columns = [
  ...
  { name: "hipaa", label: "rule.hipaa", field: "hipaa", align: "left" as const },
...
const tableRows = computed<TableRow[]>(() =>
  store.events.map((e) => {
    ...
    return {
      ...
      hipaa: (rule?.hipaa as string | string[]) ?? "",
...
.hipaa-add-filter-card {
  min-width: 280px;
}

/* Histogram */
.hipaa-events-histogram {
  padding: 8px 16px;
  background: var(--mdm-bg-card, #fff);
  border-bottom: 1px solid var(--mdm-border-light, #f0f0f0);
}

.hipaa-histogram-loading {
  display: flex;
  justify-content: center;
  padding: 8px;
}

.hipaa-histogram-label {
  text-align: center;
  font-size: 11px;
...
.hipaa-events-time-range {
  font-size: 11px;
  color: var(--mdm-text-secondary, #888);
}

/* Table */
.hipaa-events-table-wrap {
  flex: 1;
  padding: 0 16px 16px;
}
...
.hipaa-events-table :deep(.q-table__top) {
  padding: 0;
}

.hipaa-events-table :deep(tbody tr) {
  cursor: pointer;
}

.hipaa-events-table :deep(tbody tr:hover) {
...
/* Dark mode */
.body--dark .hipaa-events-toolbar,
.body--dark .hipaa-events-histogram,
.body--dark .hipaa-events-info {
