<template>
  <q-drawer
    ...
    bordered
    overlay
    class="hipaa-doc-drawer"
    @update:model-value="onDrawerToggle"
  >
    <div v-if="store.inspectedEvent" class="hipaa-doc-content">
      <!-- Header -->
      <div class="hipaa-doc-header">
        <span class="hipaa-doc-title">Document Details</span>
        <div class="hipaa-doc-actions">
          <q-btn
            flat
            dense
            no-caps
      ...
      <!-- Raw JSON view -->
      <div v-if="viewTab === 'json'" class="hipaa-doc-json">
        <pre class="hipaa-json-pre">{{ formattedJson }}</pre>
      </div>

      <!-- Table view -->
      <div v-else class="hipaa-doc-table-wrap">
        <q-list dense separator class="hipaa-doc-field-list">
          <q-item
            v-for="([key, value], idx) in flatEntries"
            :key="idx"
            class="hipaa-doc-field-item"
          >
            <q-item-section side class="hipaa-doc-field-num">
              {{ idx + 1 }}
            </q-item-section>
            <q-item-section class="hipaa-doc-field-key">
              {{ key }}
            </q-item-section>
            <q-item-section class="hipaa-doc-field-value">
              {{ formatValue(value) }}
              <q-btn
      ...
      <template v-if="store.surroundingEvents.length > 0">
        <q-separator />
        <div class="hipaa-doc-surrounding">
          <div class="hipaa-doc-surrounding-title">Surrounding Documents</div>
          <q-list dense separator>
            <q-item
              v-for="(evt, idx) in store.surroundingEvents"
              :key="idx"
              clickable
              :class="{ 'hipaa-surrounding-current': evt._id === store.inspectedEvent?._id }"
              @click="store.inspectDocument(evt)"
            >
              <q-item-section>
                <q-item-label class="hipaa-surrounding-ts">
...
/* Table view */
.hipaa-doc-table-wrap {
  flex: 1;
  overflow-y: auto;
}

.hipaa-doc-field-item {
  min-height: 32px;
...
.hipaa-doc-field-item:hover .hipaa-doc-copy-btn {
  opacity: 0.7;
}
...
.hipaa-doc-surrounding {
  max-height: 300px;
  overflow-y: auto;
  padding: 8px 0;
}

.hipaa-doc-surrounding-title {
  font-size: 13px;
  font-weight: 600;
  color: var(--mdm-text-primary, #1a1a1a);
...
.hipaa-surrounding-ts {
  font-size: 11px;
  font-family: monospace;
}

.hipaa-surrounding-current {
  background: rgba(37, 99, 235, 0.08);
}

/* Dark mode */
.body--dark .hipaa-doc-title,
.body--dark .hipaa-doc-field-key,
.body--dark .hipaa-doc-surrounding-title,
.body--dark .hipaa-json-pre {
